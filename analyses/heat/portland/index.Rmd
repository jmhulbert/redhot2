...
```{r}
ggplot(places.summary,aes(place_guess,prop.unhealthy,fill=prop.group))+geom_col() +coord_flip()   +scale_fill_manual(values=c("#7fcdbb","#fe9929"))+guides(fill=FALSE)+ labs(x="Place",y="Proportion of Unhealthy Trees",title="Places with more than 5 observations")+theme( plot.title = element_text(hjust = 0.5)) + theme_bw()
```
