...
The purpose of this page is to summarize an investigation of **tree health categories** as a *categorical* response to urban heat (temperatures) for observations from Portland, Oregon.
...
```{r}
p1 <- ggplot(data,aes(binary.tree.canopy.symptoms,fill=binary.tree.canopy.symptoms))+geom_histogram(stat="count",alpha=0.5)+theme_bw()+coord_flip()+ scale_fill_manual(name="Tree Condition",values=c("#7fcdbb","#fe9929"))+guides(fill=FALSE)
```
...
```{r}
places.summary <- data.w.hoyt %>% group_by(place_guess) %>%summarize(n=n(),n.healthy=sum(binary.tree.canopy.symptoms == "Healthy", na.rm = TRUE),n.unhealthy=sum(binary.tree.canopy.symptoms == "Unhealthy", na.rm = TRUE)) %>% mutate(prop.unhealthy =n.unhealthy/n.healthy) %>% mutate(prop.group = factor(if_else(prop.unhealthy > 0.2, "high", "low"),levels = c("low", "high")))
places.summary <- places.summary %>% filter(n>=5) %>% droplevels()
...
parks.summary <- parks %>% group_by(park) %>% summarize(n=n(),n.healthy=sum(binary.tree.canopy.symptoms == "Healthy", na.rm = TRUE),n.unhealthy=sum(binary.tree.canopy.symptoms == "Unhealthy", na.rm = TRUE)) %>% mutate(prop.unhealthy =n.unhealthy/n.healthy) %>% mutate(prop.group = factor(if_else(prop.unhealthy > 0.2, "high", "low"),levels = c("low", "high"))) %>% mutate(park = factor(park, levels = c("Willamette Park","Washington Park","Mt. Tabor Park","Hoyt Arboretum","Gabriel Park","Forest Park Conservancy","Columbia Park and Annex")))
```


```{r}
p7 <- ggplot(parks,aes(binary.tree.canopy.symptoms,pl.af.f, fill=binary.tree.canopy.symptoms ))+geom_boxplot(alpha=0.5) +facet_wrap(~park,ncol=1) +theme_bw()+coord_flip() + scale_fill_manual(name="Tree Condition",values=c("#7fcdbb","#fe9929"))+guides(fill=FALSE) +labs(x="Tree Condition",y="Temperature (F)")

p8 <- ggplot(parks.summary,aes(park,prop.unhealthy,fill=prop.group))+geom_col()+coord_flip()+theme_bw()+geom_text(aes(label=n),hjust = -0.2) + scale_fill_manual(name="Proportion",values=c("#7fcdbb","#fe9929"))+guides(fill=FALSE) + labs(y="Proportion of Trees Categorized as Unhealthy (n=total)",x="Portland Park")
```
